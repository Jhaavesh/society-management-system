...
test("login includes flatId and resident bill reads are flat-scoped", async () => {
  const login = await request(app).post("/api/auth/login").send({ email: "resident@example.com", password: "password" });
  ...
  const response = await request(app).get(`/api/maintenance?societyId=${fixtures.society._id}`).set("Authorization", `Bearer ${fixtures.residentToken}`);
  assert.equal(response.status, 200);
  assert.equal(response.body.length, 1);
  assert.equal(String(response.body[0].flatId._id || response.body[0].flatId), String(fixtures.flat._id));

  const noFlatToken = token({ sub: new mongoose.Types.ObjectId(), role: "resident", societyIds: [fixtures.society._id], flatId: null });
  const forbidden = await request(app).get(`/api/maintenance?societyId=${fixtures.society._id}`).set("Authorization", `Bearer ${noFlatToken}`);
  assert.equal(forbidden.status, 403);
});

test("resident complaint and visitor requests cannot choose another flat", async () => {
  const complaint = await request(app).post("/api/complaints").set("Authorization", `Bearer ${fixtures.residentToken}`).send({ societyId: fixtures.society._id, flatId: fixtures.otherFlat._id, category: "Security", description: "Test complaint" });
  assert.equal(complaint.status, 201);
  assert.equal(String(complaint.body.flatId), String(fixtures.flat._id));

  const visitor = await request(app).post("/api/visitors").set("Authorization", `Bearer ${fixtures.residentToken}`).send({ societyId: fixtures.society._id, flatId: fixtures.otherFlat._id, visitorName: "Guest", visitDate: new Date().toISOString() });
...
test("payments support partial balances and reject overpaying or duplicate references", async () => {
  ...
  const first = await request(app).post("/api/payments").set("Authorization", `Bearer ${fixtures.managerToken}`).send({ ...base, amountPaid: 40 });
  assert.equal(first.status, 201);
  let updated = await MaintenanceBill.findById(bill._id).lean();
  assert.equal(updated.amountPaid, 40);
  assert.equal(updated.status, "pending");

  const duplicate = await request(app).post("/api/payments").set("Authorization", `Bearer ${fixtures.managerToken}`).send({ ...base, amountPaid: 40 });
  assert.equal(duplicate.status, 409);
  const overpay = await request(app).post("/api/payments").set("Authorization", `Bearer ${fixtures.managerToken}`).send({ ...base, transactionRef: "txn-2", amountPaid: 61 });
  assert.equal(overpay.status, 400);
  const final = await request(app).post("/api/payments").set("Authorization", `Bearer ${fixtures.managerToken}`).send({ ...base, transactionRef: "txn-2", amountPaid: 60 });
